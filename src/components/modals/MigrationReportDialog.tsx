...
export function MigrationReportDialog({
    ...
    const getTypeLabel = (type: string) => {
        return synapseTypes.find(t => t.value === type)?.label ?? type;
    };
    ...
    const downloadPDF = () => {
        try {
            ...
            doc.text("Synapse Migration Report", pageWidth / 2, 22, { align: "center" });
            ...
            doc.text(`Source: Azure Synapse  |  Target: Microsoft Fabric`, pageWidth / 2, 33, { align: "center" });
            ...
            const tableData = items.map(item => {
                let details = '';
                if (item.type === 'SparkPool') {
                    if (item.runtimeVersion) details = `Runtime: ${item.runtimeVersion}`;
                    if (item.nodeType) details += details ? ` | Node: ${item.nodeType}` : `Node: ${item.nodeType}`;
                    if (item.nodes) details += details ? ` | Nodes: ${item.nodes}` : `Nodes: ${item.nodes}`;
                } else if (item.type === 'Notebook') {
                    if (item.language) details = `Language: ${item.language}`;
                } else if (item.type === 'Pipeline') {
                    if (item.activities) details = `Activities: ${item.activities}`;
                }
                // else if (item.type === 'LinkedService') {
                //     const lsType = (item as any).linkedServiceType;
                //     if (lsType) details = `Type: ${lsType}`;
                // }
                ...
                didDrawCell: (data: any) => {
                    ...
                    if (data.section === 'body' && data.column.index === 5 && data.cell.raw !== '-') {
                        ...
                        if (errorMsg.includes("already exists")) {
                            doc.setTextColor(245, 158, 11); // Amber for "already exists"
                        } else if (
                            errorMsg.toLowerCase().includes("skipped") ||
                            errorMsg.toLowerCase().includes("replaced")
                        ) {
                            doc.setTextColor(107, 114, 128); // Gray for skipped/replaced
                        } else {
                            doc.setTextColor(239, 68, 68); // Red for other errors
                        }
            ...
            const filename = `synapse-migration-report${selectionSuffix}-${timestamp}.pdf`;
